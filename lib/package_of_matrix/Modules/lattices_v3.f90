      ...
      Contains
        ...
        subroutine Make_lattice(L1_p, L2_p, a1_p, a2_p, Latt) 
          ...
          do nr = 1, Latt%N
             do nd1 = -1,1
                do nd2 = -1,1
                   d_p = nd1*a1_p + nd2*a2_p
                   x_p  = Latt%list(nr,1)*Latt%a1_p + Latt%list(nr,2)*Latt%a2_p  + d_p
	 ...
	 open(unit=250, file='nnlist.dat', status='unknown')
	 do nr = 1, Latt%N
	 do nd1 = -1,1
	 do nd2 = -1,1
		write(250,*) nr, nd1, nd2, Latt%nnlist(nr,nd1,nd2)
	 end do
	 end do
	 end do
	 close(250)
...
!********
        integer Function Inv_K(XK_P,Latt) 
          ...
          nkx = nint (Iscalar(XK_P,Latt%b1_perp_p) )
          nky = nint (Iscalar(XK_P,Latt%b2_perp_p) )
...
!********
        integer Function Inv_R(XR_P,Latt) 
          ...
          n_1 =  nint ( Iscalar(Latt%BZ1_p,XR1_p) / (2.d0*pi) )
          n_2 =  nint ( Iscalar(Latt%BZ2_p,XR1_p) / (2.d0*pi) )
